...
class SLAOptimizer:
    ...
    def optimize_schedule(self, 
        ...
        try:
            ...
            # [E11] bang-bang 双相调度(带滞回),替代连续坡道:
            #   DRAIN(积压期):长步(drain_ms)+大 chunk → prefill 吞吐拉满快速排空;
            #   STEADY(稳态):短步(steady_ms)→ decode 快;E10 配额保证新到达持续推进。
            # 连续坡道的失败(E9a/b 实证):大部分时间卡在中间步长,两头不讨好。
            if _KNOBS.get_float("VLLM_SLA_BANGBANG", 0.0) > 0:
                drain_hi = _KNOBS.get_float("VLLM_SLA_DRAIN_Q_HI", 8.0)
                drain_lo = _KNOBS.get_float("VLLM_SLA_DRAIN_Q_LO", 2.0)
                if getattr(self, "_drain_mode", False):
                    if n_waiting <= drain_lo:
                        self._drain_mode = False
                else:
                    if n_waiting >= drain_hi:
                        self._drain_mode = True
                if self._drain_mode:
                    budget_lat_ms = _KNOBS.get_float("VLLM_SLA_DRAIN_MS", 100.0)
                    self._max_chunk = _KNOBS.get_int("VLLM_SLA_DRAIN_CHUNK", 2048)
                else:
                    budget_lat_ms = _KNOBS.get_float("VLLM_SLA_STEADY_MS", 28.0)
            best_result = None
            best_e2e = float('inf')
