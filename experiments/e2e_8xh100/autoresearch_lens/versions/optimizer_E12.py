...
class _LiveKnobs:
    """live 旋钮:VLLM_SLA_KNOB_FILE 的 KEY=VAL 文件(2s TTL 缓存)> env > 默认。"""

    def __init__(self):
        self._path = _os.getenv("VLLM_SLA_KNOB_FILE", "")
        self._cache = {}
        self._mtime = 0.0
        self._checked = 0.0
    ...
    def get_float(self, name, default):
        ...
        if now - self._checked >= 2.0:
            ...
            if self._path:
                try:
                    mt = _os.path.getmtime(self._path)
                    if mt != self._mtime:
                        c = {}
                    ...
                    pass
        v = self._cache.get(name, _os.getenv(name, str(default)))
        try:
            return float(v)
        except ValueError:
...
class SLAOptimizer:
    ...
    def compute_adaptive_target_latency(self, queue_length: int) -> float:
        ...
        Returns:
            目标延迟(ms)
        """
        # [E12] 涡轮排空:深积压时直接把 target 抬到 TURBO_MS,
        # 加速冷启动/突发积压的排空(老控制器的稳态行为完全不变)。
        deep_q = _KNOBS.get_float("VLLM_SLA_DEEP_Q", 0)
        if deep_q > 0 and queue_length >= deep_q:
            turbo = _KNOBS.get_float("VLLM_SLA_TURBO_MS", 110.0)
            if self.config.verbose_logging:
                logger.debug(f"[E12] turbo drain: Q={queue_length} -> {turbo}ms")
            return turbo
        # 基于队列长度的线性插值
