...
import signal
try:
    from datasets import load_dataset  # 仅在线拉 HF 数据集时需要;读本地 JSONL 不需要
except ImportError:
    load_dataset = None
...
def process_dataset_item(item: dict, sample_start: float = 0.0, sample_end: float = 1.0, 
    ...
    try:
        ...
        if isinstance(item.get('messages'), list) and item['messages']:
            # 已是 OpenAI 格式的多轮消息(如 FlowGPT body.prompt),直接用
            messages = item['messages']
        elif ep_config.get('dataset') == 'simplescaling/s1K':
            messages = [
            ...
            # 规范化角色名:ShareGPT 的 human/gpt -> OpenAI 的 user/assistant
            _role_map = {"human": "user", "gpt": "assistant", "system": "system",
                         "user": "user", "assistant": "assistant"}
            messages = [{"role": _role_map.get(conv["from"], "user"), "content": conv["value"]}
                        for conv in conversations]
        # 每条请求的输出上限:优先用 JSONL 自带的 max_tokens,否则用全局配置
        item_max_tokens = item.get("max_tokens") or ep_config.get("max_tokens", 200)
        if ep_config.get("use_chat", True):
            if not messages:
                logger.warning(f"Empty messages for conversation {conversation_id}, skipping")
                return None
...
def log_reader_thread(input_file: str, preload_time: int = 180, sample_start: float = 0.0, 
    ...
    try:
        ...
        # 本地 JSONL(由 prepare_workload.py 生成)优先;否则回退到 HF Hub 在线加载
        if dataset and os.path.exists(dataset):
            logger.info(f"Loading local JSONL dataset: {dataset}")
            with open(dataset, 'r') as f:
                for line in f:
                    ...
                    if job:
                        job_queue.put(job)
                        job_count += 1
            logger.info(f"Finished reading local dataset, added {job_count} jobs")
            return
        if True:
            ds = load_dataset(dataset, split="train", token=os.getenv("HUGGINGFACE_TOKEN"))
    ...
    try:
        ...
        if job.use_chat:
            ...
            response = await client.chat.completions.create(
                ...
                messages=job.body.get("messages"),  # 已是 [{"role","content"},...] 列表,直接传
        ...
        logger.error(f"Request failed: {type(e).__name__}: {e!r}")
...
class ResultCollector:
    ...
    def add_detailed_result(self, request_id, conversation_id, send_time, ttft, total_time, tokens_in, tokens_out):
        ...
        if self.detailed_logs:
            ...
            # 实时写入CSV文件(含 KV cache 命中 token 数)
            cached = cached_tokens_map.pop(request_id, 0)
            if hasattr(self, 'csv_file') and not self.csv_file.closed:
                csv_line = f"{request_id},{conversation_id},{send_time},{ttft_time},{total_end_time},{tokens_in},{tokens_out},{ttft},{tpot},{cached}\n"
...
async def replay_by_qps(client, result_collector, target_qps, round_duration,
    ...
    pending_tasks = []  # 跟踪在途请求,退出前需等其完成,避免 client 被提前关闭导致 ReadError
    ...
    try:
        while True:
            ...
            if job_queue.empty():
                await asyncio.sleep(0.1)  # 给一个机会让更多任务进来
                if job_queue.empty():  # 二次确认
                    # 等待所有在途请求读完流,再收集结果并关闭 client
                    if pending_tasks:
                        logger.info(f"Waiting for {len(pending_tasks)} in-flight requests to finish...")
...
def results_analysis(
    ...
    # 防御:本轮无成功(非异常)请求 -> 下方引用未赋值的 mean_tokens_in/table 会抛
    # 异常被上层 except 吞掉 -> 到不了 max_rounds 的 os._exit -> 死循环卡住。提前返回。
    if len(cdf) == 0:
        logger.warning(
            "results_analysis: 本轮无成功请求 (total=%d, success=%d);跳过详细分析",
            total_requests, successful_requests)
        if json_output:
            try:
                rec = {"model": model, "success_rate": success_rate,
                       "total_requests": int(total_requests),
                       "successful_requests": int(successful_requests)}
                if qps is not None:
                    rec["target_qps"] = qps
                if actual_qps is not None:
                    rec["actual_qps"] = actual_qps
                json.dump(rec, json_output_f)
                json_output_f.write("\n")
                json_output_f.close()
            except Exception:
                pass
